...
(define-test-suite testiffequiv
  (check-equal?
    (iffi #f #f) (andi (impliesi #f #f) (impliesi #f #f)))
  
  (check-equal?
    (iffi #f #t) (andi (impliesi #f #t) (impliesi #t #f)))

  (check-equal?
    (iffi #t #f)(andi (impliesi #t #f) (impliesi #f #t)))
  
  (check-equal?
    (iffi #t #t) (andi (impliesi #t #t) (impliesi #t #t)))
  )
...
(define-test-suite testimpliesequiv
 (check-equal?
    (ori (noti #f) #f)(impliesi #f #f))
  
  (check-equal?
    (ori (noti #f) #t) (impliesi #f #t))

  (check-equal?
    (ori (noti #f) #t) (impliesi #f #t))
  
  (check-equal?
    (ori (noti #t) #f)(impliesi #t #f))
  
  (check-equal?
    (ori (noti #t) #t) (impliesi #t #t))
)
...
(define (andlist L)
  (if (empty? L) #t (if (equal? (first L) #t) (andlist (rest L)) #f))
)

#|
(define (orlist L)
  (if (or (cons? L) (empty? L)) #f (foldr (equal? (first L) #t) #t (andlist rest L)))
  )
|#
...
(define (allones L)
  (if (empty? L) #t
      (if (equal? (first L) 1) (allones(rest L)) #f)
      )
  )
...
(define (atleastone L)
  (if (empty? L) #f
      (if (equal? (first L) 1 ) #t (atleastone (rest L)))
      )
  )
...
(define (exactlyone L)
  (if (empty? L) #f
      (if (equal? (first L) 1 ) (if (equal? (first L) 1 ) #f (if (equal? (first L) 1 ) #f #t)) (atleastone (rest L)))
      )
...
(define (oddones L)
    (if (empty? L) (#f)
      (if (equal? (first L) 1 ) (if(exactlyone (rest L)) #f (if (exactlyone (rest L)) #f #t)) (atleastone (rest L)))
      )
